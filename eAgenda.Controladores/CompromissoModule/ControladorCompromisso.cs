...
namespace eAgenda.Controladores.CompromissoModule
{
    public class ControladorCompromisso : Controlador<Compromisso>
    {
        ...
        private const string sqlVerificarDataeHora = 
            @"SELECT COUNT(*) FROM TBCOMPROMISSO WHERE [DATA] = @DATA 
            AND 
            @HORAINICIO BETWEEN HORAINICIO 
            AND 
            HORATERMINO OR @HORATERMINO BETWEEN HORATERMINO AND HORAINICIO";
        ...
        public override string InserirNovo(Compromisso registro)
        {
            ...
            if(VerificarHora(registro.Data, registro.HoraInicio, registro.HoraTermino) == false)
            {
        ...
        private bool VerificarHora(DateTime data, TimeSpan horainicio, TimeSpan horatermino)
        {
            var parametros = new Dictionary<string, object>();

            parametros.Add("DATA", data);
            parametros.Add("HORAINICIO", horainicio.Ticks);
            parametros.Add("HORATERMINO", horatermino.Ticks);

            return Db.Exists(sqlVerificarDataeHora, parametros);
        }
    }
}
